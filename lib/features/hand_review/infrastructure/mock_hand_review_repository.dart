...
class MockHandReviewRepository implements HandReviewRepository {
  ...
  HandReviewResult analyze(HandReviewInput input) {
    ...
    final postflop =
        _analyzePostflop(input, goodPoints, improvements, alternatives, topics);
    score += postflop;

    if (goodPoints.isEmpty) {
      goodPoints.add('最後まで自分のラインを決めて打ち切れています。'
          '判断の良し悪しより先に、迷いを言語化できていることが上達の入り口です。');
    }
    if (improvements.isEmpty) {
      improvements.add('大きな問題は見当たりません。'
          '次は同じスポットで「相手のレンジに何が残っているか」を口に出して確認してみてください。');
    }

    return HandReviewResult(
      score: score.clamp(30, 97),
      summary: _summary(input, improvements.first),
  ...
  int _analyzePreflop(
    ...
    final heroActions =
        input.preflop.where((action) => action.isHero).toList(growable: false);
    if (heroActions.isEmpty) return 0;

    final chart =
        _rangeRepository.chartFor(input.tableType, input.heroPosition);
    ...
    if (firstAction == PokerActionType.limp) {
      topics.add('preflop');
      improvements.add('プリフロップでリンプ（コールだけで参加）しています。'
          '${input.heroPosition.label} から入るなら、まずレイズで主導権を取る形に統一しましょう。');
      delta -= 8;
      return delta;
    }

    final heroRaised = firstAction == PokerActionType.raise ||
    ...
    if (heroRaised) {
      switch (recommended) {
        ...
        case RangeAction.fourBet:
          goodPoints.add('${hand.code} は ${input.heroPosition.label} の'
              'レンジにきちんと入っているハンドです。プリフロップの入り方は問題ありません。');
          delta += 6;
        case RangeAction.mixed:
          goodPoints.add('${hand.code} は ${input.heroPosition.label} では境界線上のハンドです。'
              'レイズ自体は間違いではありませんが、テーブルが荒れているときは外してかまいません。');
        case RangeAction.call:
          topics.add('preflop');
          improvements.add('${hand.code} は ${input.heroPosition.label} では'
              'レイズよりもコール寄りのハンドです。レンジ表で位置づけを確認しておきましょう。');
          delta -= 4;
        case RangeAction.fold:
          topics.add('preflop');
          topics.add('position');
          improvements.add('${hand.code} は ${input.heroPosition.label} の'
  ...
  int _analyzePostflop(
    ...
    if (texture.wetness == BoardWetness.dry && bigBets > 0) {
      ...
      improvements.add('${texture.wetness.label}ボードで大きめのベットを使っています。'
          'ドローの少ないボードでは、小さいサイズを高頻度で使うほうが機能しやすいスポットです。');
      alternatives.add('同じ場面でポットの 1/3 ほどに落とすと、'
    ...
    if (texture.wetness == BoardWetness.wet && smallBets > 0 && bigBets == 0) {
      topics.add('bet_sizing');
      improvements.add('${texture.wetness.label}ボードで小さいサイズだけを使っています。'
          'ドローに安くカードを与えてしまうため、強いハンドではサイズを上げましょう。');
      alternatives.add('同じ場面でポットの 3/4 に上げると、'
    ...
    if (aggressive == 0) {
      ...
      improvements.add('ポストフロップでチェックとコールだけになっています。'
          '自分から仕掛ける回がないと、勝っている回の利益を取りきれません。');
      delta -= 5;
    } else if (aggressive >= 2) {
      goodPoints.add('複数ストリートで自分から仕掛けられています。'
          'ラインに一貫性があるのは良い点です。');
      delta += 4;
    }
  ...
  String _summary(HandReviewInput input, String mainImprovement) {
    return '${input.heroPosition.label} からの ${input.lastStreetLabel}までのハンドです。'
        '${mainImprovement.split('。').first}。ここを直すと、同じ形のハンドがまとめて良くなります。';
  }
  ...
  String _practicalAdjustment(HandReviewInput input) {
    return switch (input.villainProfile) {
      VillainProfile.tight => '相手がタイトなので、ブラフの成功率が上がります。'
          '一方でコールしてきたときは本物であることが多いので、薄いバリューは減らします。',
      VillainProfile.loose => '相手がルーズなので、バリューを厚く、ブラフを薄くします。'
          '相手のレンジが広いぶん、こちらの中程度のハンドの価値も上がります。',
      VillainProfile.passive => '相手が受け身なので、相手からのベットは強さのサインとして重く見ます。'
          'こちらから仕掛ける回を増やすのが有効です。',
      VillainProfile.aggressive => '相手が攻撃的なので、強いハンドでチェックして打たせる形が機能します。'
          'ブラフキャッチの基準も少し緩めてかまいません。',
      VillainProfile.unknown => '相手の情報がないので、まずは標準的なラインを選ぶのが安全です。'
